...
public enum HttpResponseStatus implements ResponseStatus {
    ...
    private HttpResponseStatus(final Integer code, final String message) {
        if (code < -1) {
            throw new IllegalArgumentException("code: " + code + " (expected: 0+ and !=-1) ");
    ...
    public static HttpResponseStatus valueOf(final int code) {
        if (code <= 0) {
            ...
            HttpResponseStatus[] var1 = values();
            int var2 = var1.length;

            for(int var3 = 0; var3 < var2; ++var3) {
                HttpResponseStatus status = var1[var3];
                if (status.code == code) {
                    return status;
                }
            }
